...
sys.path.append('../')
from DRL.carla import image_converter
from DRL.carla import sensor
from DRL.carla.client import make_carla_client, VehicleControl
...
from DRL.carla.util import print_over_same_line
...
class CarlaGame(object):
    ...
    def _on_render(self):
        ...
        if self._main_image is not None:
            ...
            # print('img', array.max())
            # img = array.transpose(1,0,2)
            img = Image.fromarray(array.astype('uint8'))

            saveName = '%06d_' % self._frame
            saveName += 'None_'

            if self.last_x != self._position[0] or self.last_y != self._position[1]:
                saveName = saveName + '%.6s_'% self._position[0] + '%.6s'% self._position[1]
                # saveName = saveName + '%.6s_'% self.hud.velocity[0] + '%.6s_'% self.hud.velocity[1]  + '%.6s'% self.hud.velocity[2]
                self.last_x = self._position[0]
                self.last_y = self._position[1]

                saveName += '.png'
                print(saveName)
                img.save('../dataset/carla_rawdata3/' + saveName)
